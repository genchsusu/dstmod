...
local function onharvest(inst, picker, produce)
    ...
    if not inst:HasTag("burnt") then
        ...
        then
            -- inst.components.childspawner:ReleaseAllChildren(picker) -- Gin 采集蜂蜜不会释放蜜蜂
        end
    end
end
...
local function MakeBeebox(name, common_postinit, master_postinit)
    ...
    local function fn()
        ...
        WorldSettings_ChildSpawner_SpawnPeriod(inst, 1, TUNING.BEEBOX_ENABLED)
        WorldSettings_ChildSpawner_RegenPeriod(inst, 1, TUNING.BEEBOX_ENABLED)
