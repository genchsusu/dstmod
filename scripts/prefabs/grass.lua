...
local function ontransplantfn(inst)
    -- inst.components.pickable:MakeBarren() -- Gin 移植后不会枯萎
    makemorphable(inst)
    inst.components.worldsettingstimer:StartTimer("morphdelay", GetRandomWithVariance(TUNING.GRASSGEKKO_MORPH_DELAY, TUNING.GRASSGEKKO_MORPH_DELAY_VARIANCE))
end
...
local function grass(name, stage)
    local function fn()
        ...
        inst.components.pickable:SetUp("cutgrass", TUNING.GRASS_REGROW_TIME, 10) -- Gin 0 -> 10
        ...
        -- MakeNoGrowInWinter(inst) -- Gin 冬天也能生长
