...
local function TryAddToStack(item, target)
    ...
    and item.components.stackable and not item.components.stackable:IsFull() then
...
local function AutoStack(inst)
    ...
    and inst.components.inventoryitem.owner == nil and not inst.components.stackable:IsFull() then
